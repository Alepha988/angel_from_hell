...
window = Tk()
window.title("Моргунов Алексей Сергеевич")
window.geometry("500x350")
tab_control = ttk.Notebook(window)
tab1 = ttk.Frame(tab_control)
tab2 = ttk.Frame(tab_control)
tab3 = ttk.Frame(tab_control)
tab_control.add(tab1, text="Кадькулятор")
...
def clicked1():
    if selected.get() == 0: messagebox.showinfo("Вы выбрали", "Вы выбрали первый вариант")
    if selected.get() == 1: messagebox.showinfo("Вы выбрали", "Вы выбрали второй вариант")
    if selected.get() == 2: messagebox.showinfo("Вы выбрали", "Вы выбрали третий вариант")
...
spin = Spinbox(tab1, from_=-100000, to=100000, width=5, justify="center", textvariable=var)
...
spin1 = Spinbox(tab1, from_=-100000, to=100000, width=5, justify="center", textvariable=var1)
...
btn1 = Button(tab2, text="Узнать результат", command=clicked1)
btn1.grid(column=0, row=1)

text = scrolledtext.ScrolledText(tab3, width=50, height=10)
text.grid(row=0, column=0, columnspan=2, padx=35)

btn2 = Button(tab3, text="Загузить", command=clicked2)
btn2.grid(row=1, column=0, pady=10)
btn3 = Button(tab3, text="Сохранить", command=clicked3)
